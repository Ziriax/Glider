...
namespace FlightSim.NET
{
    ...
    */
    ...
    {
        public float fMass;            // total mass (constant)
        public Matrix3x3 mInertia;     // mass moment of inertia @in body coordinates (constant)
        public Matrix3x3 mInertiaInverse;// inverse of mass moment of inertia matrix	(constant)

        public Vector vPosition;       // position @in earth coordinates
        public Vector vVelocity;       // velocity @in earth coordinates
        public Vector vVelocityBody;   // velocity @in body coordinates
        public Vector vAngularVelocity;// angular velocity @in body coordinates
        public Vector vEulerAngles;    // Euler angles @in body coordinates
        public float fSpeed;           // speed (magnitude of the velocity)

        public Quaternion qOrientation;    // orientation @in earth coordinates
        //Matrix3x3	mRotation;		// rotation matrix

        public Vector vForces;     // total force on body
        public Vector vMoments;        // total moment (torque) on body

        public Matrix3x3 mIeInverse;       // inverse of moment of inertia @in earth coordinates
    ...
    {
        public float fMass;
        public Vector vDCoords;
        public Vector vCGCoords;
        public Vector vLocalInertia;
    ...
    public class Glider
    {
        const float _DTHRUST = 10.0f;
        const float _MAXTHRUST = 3000.0f;

        public RigidBody Airplane;     // Rigid body properties of our airplane
        public BodyElement[] Element = new BodyElement[8];     // Mass, inertia and lifting surface properties of our airplane
        public Vector Thrust;          // Thrust vector, assumed to act through the plane's CG
        public float ThrustForce;  // Magnitude of the thrust force
        public bool Stalling;      // Flag to let us know if we are @in a stalled condition
        public bool Flaps;         // Flag to let us know if the flaps are down
        ...
        // properties.
        //------------------------------------------------------------------------//
        public Glider(Vector initialPosition = default(Vector))
        {
        ...
        public void CalcAirplaneMassProperties()
        {
            ...
            Element[0].vDCoords = new Vector(14.5f, 12.0f, 2.5f);
            Element[0].vLocalInertia = new Vector(13.92f, 10.50f, 24.00f);
            ...
            Element[1].vDCoords = new Vector(14.5f, 5.5f, 2.5f);
            Element[1].vLocalInertia = new Vector(21.95f, 12.22f, 33.67f);
            ...
            Element[2].vDCoords = new Vector(14.5f, -5.5f, 2.5f);
            Element[2].vLocalInertia = new Vector(21.95f, 12.22f, 33.67f);
            ...
            Element[3].vDCoords = new Vector(14.5f, -12.0f, 2.5f);
            Element[3].vLocalInertia = new Vector(13.92f, 10.50f, 24.00f);
            ...
            Element[4].vDCoords = new Vector(3.03f, 2.5f, 3.0f);
            Element[4].vLocalInertia = new Vector(0.837f, 0.385f, 1.206f);
            ...
            Element[5].vDCoords = new Vector(3.03f, -2.5f, 3.0f);
            Element[5].vLocalInertia = new Vector(0.837f, 0.385f, 1.206f);
            ...
            Element[6].vDCoords = new Vector(2.25f, 0.0f, 5.0f);
            Element[6].vLocalInertia = new Vector(1.262f, 1.942f, 0.718f);
            ...
            Element[7].vDCoords = new Vector(15.25f, 0.0f, 1.5f);
            Element[7].vLocalInertia = new Vector(66.30f, 861.9f, 861.9f);
            ...
            for(i = 0; i < 8; i++)
            {
			    @in = MathUtil.DegreesToRadians(Element[i].fIncidence);
                di = MathUtil.DegreesToRadians(Element[i].fDihedral);
                Element[i].vNormal = new Vector((float)Math.Sin(@in), (float)(Math.Cos(@in) * Math.Sin(di)), (float)(Math.Cos(@in) * Math.Cos(di)));
            ...
            vMoment = new Vector(0.0f, 0.0f, 0.0f);
            for(i = 0; i < 8; i++)
            {
        ...
        public void CalcAirplaneLoads()
        {
            ...
            for(i = 0; i < 8; i++) // loop through the seven lifting elements skipping the fuselage
            {
                ...
                {           // thus you have to recalculate the normal vector
                    float @in, di;
                    @in = MathUtil.DegreesToRadians(Element[i].fIncidence); // incidence angle
                    di = MathUtil.DegreesToRadians(Element[i].fDihedral);  // dihedral angle
                    Element[i].vNormal = new Vector((float)Math.Sin(@in),
                        (float)(Math.Cos(@in) * Math.Sin(di)),
                        (float)(Math.Cos(@in) * Math.Cos(di))); // surface normal vector
                    Element[i].vNormal.Normalize();
                }
                ...
                vDragVector = fLocalSpeed > 1 ? -vLocalVelocity / fLocalSpeed : new Vector();
                ...
                if(tmp > 1) tmp = 1;
                if(tmp < -1) tmp = -1;
                fAttackAngle = MathUtil.RadiansToDegrees((float)Math.Asin(tmp));

                // Determine the resultant force (lift and drag) on the element.
                tmp = 0.5f * MathUtil.rho * fLocalSpeed * fLocalSpeed * Element[i].fArea;
                if(i == 6) // Tail/rudder
                {
            ...
            Airplane.vForces.z += MathUtil.g * Airplane.fMass;
        ...
        * dt;
        ...
        //
        //------------------------------------------------------------------------//
        public Vector GetBodyZAxisVector()
        {
        ...
        //
        //------------------------------------------------------------------------//
        public Vector GetBodyXAxisVector()
        {
        ...
        //
        //------------------------------------------------------------------------//
        public Matrix3x3 MakeAngularVelocityMatrix(Vector u)
        {
            return new Matrix3x3(0.0f, -u.z, u.y,
                u.z, 0.0f, -u.x,
                -u.y, u.x, 0.0f);
        }
        ...
        //------------------------------------------------------------------------//
        public float LiftCoefficient(float angle, int flaps)
        {
        ...
        //  for a symmetric (no camber) airfoil without flaps.
        //------------------------------------------------------------------------//
        public float RudderDragCoefficient(float angle)
        {
